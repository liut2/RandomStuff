...
extern void preempt(unsigned int cpu_id) {
  pcb_t* procToBePreempted;

  pthread_mutex_lock(&current_mutex);
  procToBePreempted =  current[cpu_id];
  pthread_mutex_unlock(&current_mutex);

  if (procToBePreempted != NULL) {
    procToBePreempted->state = PROCESS_READY;
  }
  addReadyProcess(procToBePreempted);
  schedule(cpu_id);
}
